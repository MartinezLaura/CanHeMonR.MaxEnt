training_pnt_filename <- '/HDD/visual_interpretation/visual_interpretation_ADS/AY_07.shp'

prediction_path <- "/CID/data/cid-bulk22/Shared/projectData/CANHEMON/tmp/AY_07-2/"

conf_dir <- '/HDD/Maxent/ADS/By_Chunks/AY_07/thresh2/'
# training_pnt_filename <- '/HDD/visual_interpretation/visual_interpretation_ADS/AY_07.shp'
# prediction_path <- "/CID/data/cid-bulk22/Shared/projectData/CANHEMON/tmp/AY_07/"
# conf_dir <- '/HDD/Maxent/ADS/By_Chunks/AY_07/thresholds/'
# prefix <- "Maxent-AY_07-"
prefix <- "Maxent-AY_07-"
radius <- 2
tt<-data.frame()
#treat the txt file if needed
bu = read.table('/HDD/Lists/PBtiles/AY_07.txt')
bu = unlist(bu$V1)
bu = as.vector(bu)
...
max_permittable_cutoff <- errs_long %>% group_by(class) %>% filter( error_type == 'false_neg') %>% filter( error < 10)
...
#r_pred_dir <- '/HDD/trash/'
outputpath = '/HDD/Maxent/ADS/By_Chunks/AY_07/shp2/'

CanHeMonR.MaxEnt::classify_maxent_output_based_on_error_stats(r_pred_dir,
                                            tile = 'ALL',
