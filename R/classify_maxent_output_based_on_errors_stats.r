...
#' @description Classify maxent output to binary presence absence maps based on thresholds of pixel-level probability, expected minimum crown size, and clump size, finally outputs a polygon shapefile.
...
#' @param max_permittable_cutoff_final Cutoff-level for Maxent probability output. Typically produced by cut_off_selection.r
...
#' @param outp_dir The directory where you want to save the output shapefiles.
#' @return A shapefile for each binarized raster file
#' @seealso Depends on: \code{\link[CanHeMonR.MaxEnt]{cut_off_selection.r}}
#' @export
