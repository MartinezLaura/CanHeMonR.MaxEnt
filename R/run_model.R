...
#' @param predictor_dir Path where predictor layers are held, rasters. If loop = TRUE predictor_dir is a path
#' if loop = FALSe predictor_dir is the path plus the image to predict
...
#' @param loop Boolean. Parameter to tell if you want to process all tiles in a loop or individually, for EOS cluster loop = FALSE
#' @return A raster image for each tile with the probabilities or cummulative probabilities of presence for each class
#' @seealso Depends on: \code{\link[CanHeMonR]{calibrate_model.r}}
#' @examples \dontrun{
...
run_model <- function(predictor_dir,
  ...
  if (loop == TRUE){
    cat('Processing in a loop all images\n')
    ...
    for (i in 1:length(all_tifs)){
      ...
      tifs <- list.files(text_train_dir, recursive = F, full.names = T, pattern = ".tif")
